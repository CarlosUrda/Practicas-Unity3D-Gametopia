...
public class Interface : MonoBehaviour 
{
	...
	public GUIStyle guiTarget;        // Estilo del texto que aparece cuando se apunta al objetivo.
	public GUIStyle guiEnd;           // Estilo del texto que aparece cuando acabe la partida.
	...
	public LayerMask layerTarget;     // Capas de los objetos que afectarán al punto de mira de la cámara.
	public bool bEndGame;             // Flag para saber si se ha terminado la partida
	...
	CameraMotion cameraMotion;                   // Componente CameraMotion de la cámara
	...
	float fEndTime;                   // Tiempo transcurrido desde que acaba la partida.
	...
	const string sGamesWon = "GamesWon";  // Preferencia donde se guarda el número de partidas ganadas
	...
	// Dimensiones del texto indicando que hay un objetivo a la vista
	const int iWidthLabelTarget = 300;
	const int iHeightLabelTarget = 70;
	const int iXLabelTarget = iWidthScreen / 2 - iWidthLabelTarget / 2;
	const int iYLabelTarget = 50;

	// Dimensiones de la mirilla que se activa si hay un objeto a la vista
	const int iWidthTarget = 25;
	const int iHeightTarget = 25;
	const int iXTarget = iWidthScreen / 2 - iWidthTarget / 2;
	const int iYTarget = iHeightScreen / 2 - iHeightTarget / 2;
	...
	/*************************************************************************
	 * FUNCION: void CheckTarget()                                           *
	 * DESCRIPCION: Función encargada de comprobar si hay un objeto en el    *
	 *   punto de mira de la cámara (centro de la pantalla) para poder       *
	 *   dispararle.                                                         *
	 * ARGUMENTOS:                                                           *
	 *   ENTRADA: Ninguno.                                                   *
	 *   SALIDA: Ninguno.                                                    *
	...
	void CheckTarget()
	{
		...
		if (Physics.Raycast( cameraRay, out info, 1000, layerTarget))
		{
			GUI.color = Color.red;
			GUI.Label( new Rect( iXLabelTarget, iYLabelTarget, iWidthLabelTarget, iHeightLabelTarget), 
				       "TARGET!", guiTarget);
			GUI.DrawTexture( new Rect( iXTarget, iYTarget, iWidthTarget, iHeightTarget), energy);
		}
	...
	void OnGUI()
	{
		...
		// Si la perspectiva es en primera persona se comprueba si está apuntando algún objetivo
		if (cameraMotion.cameraPersp == CameraMotion.CameraPersp.first)
		{
			CheckTarget();			
		}
		
		// Se comprueba si ha acabado la partida al destruir el objetivo
		if (bEndGame == true)
		{
			// Se carga una nueva partida una vez pasados unos segundos
			if (fEndTime >= 5.0)
			{
				Application.LoadLevel( "Ejercicio6");
			}
			
			// Si acaba de terminar la partida se actualiza el número de partidas ganadas
			if (fEndTime == 0.0f)
			{
				// Se guarda el número de partidas ganadas
				PlayerPrefs.SetInt( sGamesWon, PlayerPrefs.GetInt( sGamesWon) + 1);
			}

			GUI.Label( new Rect( iXLabelTarget, iYLabelTarget, iWidthLabelTarget, iHeightLabelTarget), 
				       PlayerPrefs.GetInt( sGamesWon) + " WON", guiTarget);
